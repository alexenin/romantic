    ...
    class Truckers : Script
    {
        ...
        public void OnResourceStart()
        {
            try
            {
                for (int i = 0; i < getProduct.Count; i++)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Нажмите Е что-бы получить Прицеп."), new Vector3(getProduct[i].X, getProduct[i].Y, getProduct[i].Z + 0.7), 30f, 0.5f, 0, new Color(255, 255, 255));
                }
        ...
        public static void onEntityEnterDropTrailer(ColShape shape, Client player)
        {
            try
            {
                ...
                var max = Convert.ToInt32(9000 * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
                var min = Convert.ToInt32(2000 * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
