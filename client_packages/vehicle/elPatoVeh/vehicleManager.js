...
mp.events.add({
    ...
    'playerEnterVehicle': () => {
        mp.game.graphics.notify(`Нажмите ~b~F4 ~w~для открытия меню ремня безопасности.`);
    },
...
mp.keys.bind(0x73, true, _ => {
    if (state === 0 && localPlayer.vehicle) {
        if (UI === null) {
            mp.gui.cursor.visible = true;
            UI = mp.browsers.new('package://vehicle/elPatoVeh/Manager/index.html')
            state = 1;
            updateStatus();
        } else {
            mp.gui.cursor.visible = true;
