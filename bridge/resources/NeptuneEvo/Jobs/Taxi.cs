...
namespace NeptuneEvo.Jobs
{
    class Taxi : Script
    {
        ...
        private static int taxiRentCost = 350;
            ...
            if (Main.Players[player].WorkID == 3)
            {
                if (NAPI.Data.GetEntityData(player, "WORK") != null)
                {
                    ...
                    if (NAPI.Data.GetEntityData(vehicle, "TYPE") == "TAXI")
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не можете установить цену выше 2000$ или ниже 20$", 3000);
                            return;
                        }
